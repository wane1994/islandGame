...
- есть оружие, прицел и видимая пуля;
- врагов можно убивать выстрелами;
- после убийства враги появляются снова;
...
### IslandPlayerWeapon.cs

Отвечает за оружие игрока.

Скрипт:

- создаёт визуальную модель оружия;
- стреляет по левой кнопке мыши;
- использует центр камеры как направление выстрела;
- создаёт видимую пулю и след;
- наносит урон врагам;
- показывает вспышку попадания.
...
В текущей версии камера смотрит не прямо в тело игрока, а в точку немного впереди него.
Это нужно, чтобы прицел в центре экрана показывал пространство перед героем, а не был визуально приклеен к персонажу.
...
Когда враг получает смертельный урон, он вызывает `EnemyKilled()` у `IslandGameManager`.
Менеджер ждёт небольшую паузу и просит `IslandTerrainGenerator` создать нового врага:
...
Так количество врагов в игре восстанавливается после убийств.

## 11. Как работает оружие

Оружие находится в скрипте `IslandPlayerWeapon`.

Генератор добавляет его игроку автоматически:
...
- создаёт простую модель пистолета из примитивов;
- стреляет левой кнопкой мыши;
- использует луч из центра камеры;
- использует `SphereCast`, чтобы попадать по врагам чуть проще;
- создаёт видимую светящуюся пулю;
- рисует след пули;
- создаёт вспышку в точке попадания;
- наносит врагу урон.

Направление выстрела берётся из центра экрана:

```csharp
playerCamera.ViewportPointToRay(new Vector3(0.5f, 0.5f, 0f));
```

Поэтому прицел на экране показывает, куда полетит пуля.

## 12. Как работает прицел

Прицел рисуется в `IslandGameManager` через `OnGUI`.

Он находится в центре экрана:
...
Прицел не является 3D-объектом в мире.
Это HUD-элемент поверх камеры.

Из-за этого он всегда остаётся по центру экрана и показывает направление луча, которым стреляет оружие.

## 13. Как работает здоровье
...
## 15. Как сохраняется лучший результат
...
## 19. Как повторить проект с нуля
...
14. Потом добавь оружие, прицел и выстрелы по врагам.

15. Потом добавь HUD, мини-карту, звуки и сохранение лучшего времени.
...
- есть ли у игрока компонент `IslandPlayerWeapon`;
- нажата ли левая кнопка мыши;
- захвачена ли мышь в Game view;
- не слишком ли большая скорость `bulletSpeed`;
- не слишком ли маленький размер `Visible Bullet`.

### Прицел висит на герое

Это значит, что камера смотрит слишком близко в тело игрока.

В `IslandPlayerController` нужно, чтобы `cameraLookOffset` был сдвинут вперёд:
...
- вызывается ли `TakeDamage()` в `IslandEnemy`;
- достаточно ли большой `damage` у оружия;
- не мешает ли попаданию слишком маленький `hitRadius`;
- есть ли у врага компонент `IslandEnemy`.
...
- добавить разные виды оружия или способности;
- добавить патроны и перезарядку;
